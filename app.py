...
class DynamicLeadScoringApp:
    def __init__(self):
        ...
        # Load model predictor (must exist)
    ...
    def render_scoring_section(self):
        ...
        if not hasattr(st.session_state, "ready_for_scoring") or not st.session_state.ready_for_scoring:
            # Allow scoring if manual data exists
            if not self.session_state.user_input_data:
                return

            # Build features from manual data only
            self.session_state.final_features = self.feature_builder.build_features(
                linkedin_data={},
                company_data={},
                user_data=self.session_state.user_input_data
            )
    ...
    def run(self):
        ...
        # Sidebar first (so manual data is available before scoring)
        self.render_sidebar()
        ...
        with side_col:
            ...
            st.write("Model Loaded:", True)
            st.write("Ready for Scoring:", getattr(st.session_state, "ready_for_scoring", False))
